...
import type { IAnswer, IQuestion } from "@/types/Post"
  ...
  return (
    <div className="space-y-6">
        ...
        <CardFooter className="flex justify-between items-center">
          <div className="flex items-center space-x-4">
            <Button onClick={user_id ? handleUpvote : () => {}} variant="ghost" size="sm">
              <ThumbsUp className={"mr-1 h-4 w-4 " + (vote?.voteType == 0 && "fill-current text-blue-500")} />
            </Button>
            <p>{reputation}</p>
            <Button onClick={user_id ? handleDownVote : () => {}} variant="ghost" size="sm">
              <ThumbsDown className={"mr-1 h-4 w-4 " + (vote?.voteType == 1 && "fill-current text-orange-500")} />
            </Button>
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-muted-foreground">
              Posted by{" "}
              <Link className="hover:underline" href={`/user/${question!.postedBy.userName}`}>
                u/{question?.postedBy.userName}
              </Link>{" "}
              {question && formatDate(question.postedAt.toString())}
            </span>
          </div>
        </CardFooter>
      </Card>
      ...
      {question && question.id && (
        <ReplyList discussionId={question.id} replies={answers} isOwner={isOwner && !question.isAnswered} />
      )}
